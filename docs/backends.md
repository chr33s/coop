...
The fork's own machine integration tests cover the extension: attachment to only the selected network across restarts, boot failure when that network is deleted, no agent socket or `SSH_AUTH_SOCK` in the guest with `--no-ssh-agent` even when the host sets one, the `machine inspect` fields, and first boot through `machine run` without a terminal (as coop boots). `testSeparateNetworksAreIsolated` checks cross-network isolation on macOS 27: a machine on one network cannot reach a machine on another by TCP or ICMP over IPv4 or IPv6, including after adding an on-link route or an address inside the other network's subnet, while a machine on the target's own network can. coop's end-to-end flow has run against the fork (`1.4.1+coop.707eb44`, macOS 27): `setup` builds and verifies the image; `up` creates the instance behind the isolation gate, pins the host key, copies the workspace, and bootstraps Claude Code and Codex (config directories copied; API keys reach sessions through the environment and appear nowhere on the guest disk); `stop`/`start` moves the machine to a new address and backing container and accepts the unchanged pinned key; a guest-generated host key makes the next `start` fail with `APPLE_HOST_KEY_CHANGED` without re-enrolling; after the VM process is killed, `status` reports the instance stopped and `start` recovers it; and when coop itself is killed right after machine creation, `status` and `start` report `APPLE_OPERATION_UNCERTAIN` and `destroy` removes exactly the journaled machine and network. Still untested: neighbor impersonation against host-to-peer SSH, UDP/broadcast/multicast, whether forwarded ports and the credential proxy are reachable from another guest, killing coop at the other journaled stages, and endurance.
