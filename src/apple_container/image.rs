...
/// would fight it (or need hardware the VM lacks) are masked. `e2fsprogs`
/// stays installed: disk growth runs `resize2fs` from a clone of this image.
...
mod tests {
    ...
    #[test]
    fn image_keeps_e2fsprogs_for_disk_growth() {
        assert!(dockerfile().contains(" e2fsprogs"));
    }
}
