...
| `image-inspect-coop-707eb44.json` | Captured: `container image inspect ubuntu:24.04` from the `vendor/container` fork at `707eb44`, 2026-09-25. The digest is under `configuration.descriptor`. |
| `machine-inspect-extended.json` | Synthetic. Shape follows `InspectOutput` in `Sources/ContainerCommands/Machine/MachineInspect.swift` of the fork at `fdddb59`: the 1.4.1 fields plus `network` (null for the built-in network; omitted by builds before `83b256f`) and `sshAgentForwarding`. |
